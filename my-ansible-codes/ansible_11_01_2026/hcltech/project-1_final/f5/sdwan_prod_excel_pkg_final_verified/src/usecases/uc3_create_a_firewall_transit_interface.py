"""
Use Case 3: Create a Firewall Transit Interface
"""
...
UC_KEY = "uc3"
UC_NAME = "Use Case 3: Create a Firewall Transit Interface"


class UC3CreateAFirewallTransitInterface:
    def __init__(
        self,
    ...
    def run(self, site: SiteConfig, dry_run: bool = False, force: bool = False) -> UCResult:
        ...
        try:
            template_data, changed = self._apply_uc3_changes(site, template_data, persist=not dry_run)
        except Exception as exc:
    ...
    def _apply_uc3_changes(
        ...
        vpn0_noip_tid = site.vpn0_noip_template_id or self._find_template_id(
            site.vpn0_noip_template_name, "cisco_vpn_interface"
        )
        if vpn0_noip_tid and vpn0_noip_tid not in template_str:
            changed = self._attach_under_named_vpn(template_data, vpn0_noip_tid, 0) or changed

        transit_tid = site.firewall_transit_template_id or (
            self._find_template_id(site.firewall_transit_template_name, "cisco_vpn_interface")
            if site.firewall_transit_template_name
            else ""
        )
        if transit_tid and transit_tid not in template_str:
            logger.warning("Firewall transit sub-interface template is not attached to the device template")
    ...
    def _apply_variables(self, row: Dict[str, Any], site: SiteConfig) -> Dict[str, Any]:
        for key, value in site.uc3_variables.items():
    ...
    def _attach_under_named_vpn(self, template_data: Dict[str, Any], template_id: str, vpn_id: int) -> bool:
        for block in template_data.get("generalTemplates", []):
            ...
            if actual_vpn_id != vpn_id:
                continue
            sub_templates = block.setdefault("subTemplates", [])
            if template_id in [item.get("templateId") for item in sub_templates]:
                return False
            sub_templates.append({"templateId": template_id, "templateType": "cisco_vpn_interface"})
            return True
        raise TemplateNotFoundError(template_name=f"VPN {vpn_id} feature template")
